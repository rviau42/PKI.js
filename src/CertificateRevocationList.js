...
export default class CertificateRevocationList {
	...
	isCertificateRevoked(certificate) {
		...
		//region Check that issuer of the input certificate is the same with issuer of this CRL and CRL is not indirect
		if (this.issuer.isEqual(certificate.issuer) === false && !isIndirect)
			return false;
		//endregion
		...
		// According to RFC5280, if CRL is Indirect:
		// - a revoked certificate should have the CRL Entry Extension "Certificate Issuer" ("2.5.29.29") by its DN (General name: type 4)
		// - by default, process MUST consider as the same as the previous certificate in the list
		// - by default, process MUST consider the CRL issuer as the certificate issuer
		let revokedIssuer = this.issuer
		for (const revokedCertificate of this.revokedCertificates) {
			if (isIndirect) {
				const tmpIssuer = revokedCertificate.crlEntryExtensions?.extensions?.find(({ extnID }) => extnID === "2.5.29.29")
				if (tmpIssuer) {
					const gn = tmpIssuer.parsedValue.names.find(gn => gn.type === 4)
					if (gn) {
						revokedIssuer = gn.value
					}
				}
			}
